...
def register_tools(mcp: FastMCP):
    ...
    def list_drives():
        return handle_list_drives()

    @mcp.tool(name="list_children")
    def list_children(path: str = ""):
        return handle_list_children(path)

    @mcp.tool(name="resolve_path_to_item")
    def resolve_path_to_item(path: str):
        return handle_resolve_path_to_item(path)

    @mcp.tool(name="search_items")
    def search_items(query: str):
        return handle_search_items(query)

    @mcp.tool(name="read_text_file")
    def read_text_file(path: str):
        return handle_read_text_file(path)

    @mcp.tool(name="download_file")
    def download_file(path: str):
        return handle_download_file(path)

    @mcp.tool(name="get_item_versions")
    def get_item_versions(path: str):
        return handle_get_item_versions(path)
    ...
    def upload_file_small(path: str, content_b64: str, conflict: str = "replace"):
        return handle_upload_file_small(path, content_b64, conflict)
    ...
    def rename_item(path: str, new_name: str):
        return handle_rename_item(path, new_name)

    @mcp.tool(name="copy_item")
    def copy_item(source: str, destination: str):
        return handle_copy_item(source, destination)
